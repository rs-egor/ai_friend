...
from app.utils.datetime import utcnow, utcnow_plus, is_expired
...
class SubscriptionService:
    ...
    async def check_message_limit(self, user: User) -> tuple[bool, int, int]:
        ...
        if subscription and subscription.is_premium:
            ...
            if subscription.expires_at and is_expired(subscription.expires_at):
    ...
    async def activate_premium(
        ...
        if plan_type == "yearly":
            subscription.expires_at = utcnow_plus(days=365)
        else:  # monthly
